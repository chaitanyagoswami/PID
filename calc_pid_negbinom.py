...
class calc_NB_PID:
    ...
    def _calc_analytical_q(self):
        ...
        if np.min(f2M)<=10 or self.bias_grid:
            minf2_lst = np.linspace(0,0.99*np.min(f2M),10)
        else:
        ...
        for minf2 in list(minf2_lst):
            ...
            if upper_bias_lim<=10 or len(minf2_lst)<=3 or self.bias_grid:
                minf1_lst = np.linspace(0,0.99*upper_bias_lim,10)
            else:
                ...
                if np.abs(minf1) <= 1e-03 and np.abs(minf2)<=1e-03:
                        ...
                        for ii in range(qMXY.shape[1]):
                            for iii in range(qMXY.shape[2]):
                                if self.f1(self.M[i])-minf1>=self.f2(self.M[i])-minf2-1e-02:
                                    a, b  = self.f2(self.M[i]), self.f1(self.M[i])-self.f2(self.M[i])
                                    qMXY[i,ii,iii] = qMXY[i,ii,iii]*pM[i]*nbinom.pmf(k=self.X[ii], n=int(self.f1(self.M[i])), p=self.p)*betabinom.pmf(k=self.Y[iii],n=self.X[ii], a=a, b=b)
                                else:
                                    print('fuck')
                                    a, b = int(self.f1(self.M[i])), int(self.f2(self.M[i])-self.f1(self.M[i]))
                                    qMXY[i,ii,iii] =  qMXY[i,ii,iii]*pM[i]*nbinom.pmf(self.Y[iii], n=int(self.f2(self.M[i])), p=self.p)*betabinom.pmf(k=self.X[ii],n=self.Y[iii], a=a, b=b) 
                    ...
                    for i in range(qMABCD.shape[0]):
                        for ii in range(qMABCD.shape[1]):
                            for iii in range(qMABCD.shape[3]):
                                if self.f1(self.M[i])-minf1>=self.f2(self.M[i])-minf2-1e-02:
                                    a, b  = self.f2(self.M[i])-minf2, (self.f1(self.M[i])-minf1)-self.f2(self.M[i])
                                    qMABCD[i,ii,:,iii] = qMABCD[i,ii,:,iii]*pM[i]*nbinom.pmf(self.X[ii], n=int(self.f1(self.M[i])-minf1), p=self.p)*betabinom.pmf(k=self.Y[iii],n=self.X[ii], a=a, b=b)
                                else:
                                    print('fuck')
                                    a, b = (self.f1(self.M[i])-minf1), (self.f2(self.M[i])-minf2)-(self.f1(self.M[i])-minf1)
                                    qMABCD[i,ii,:,iii] =  qMABCD[i,ii,:,iii]*pM[i]*nbinom.pmf(self.Y[iii], n=self.f2(self.M[i])-minf2, p=self.p)*betabinom.pmf(k=self.X[ii],n=self.Y[iii], a=a, b=b)
                
                    indep_noise2 = nbinom.pmf(self.X, n=minf1, p=self.p).reshape([1,1,qMABCD.shape[2],1])
                    ...
                    for i in range(qMABCD.shape[0]):
                        for ii in range(qMABCD.shape[1]):
                            for iii in range(qMABCD.shape[3]):
                                if self.f1(self.M[i])-minf1>=self.f2(self.M[i])-minf2-1e-02:
                                    a, b  = (self.f2(self.M[i])-minf2), (self.f1(self.M[i])-minf1)-(self.f2(self.M[i])-minf2)
                                    qMABCD[i,ii,iii,:] = qMABCD[i,ii,iii,:]*pM[i]*nbinom.pmf(self.X[ii], n=self.f1(self.M[i])-minf1, p=self.p)*betabinom.pmf(k=self.Y[iii],n=self.X[ii], a=a, b=b)
                                else:
                                    print('fuck')
                                    a, b = (self.f1(self.M[i])-minf1), (self.f2(self.M[i])-minf2)-(self.f1(self.M[i])-minf1)
                                    qMABCD[i,ii,iii,:] =  qMABCD[i,ii,iii,:]*pM[i]*nbinom.pmf(self.Y[iii], n=self.f2(self.M[i])-minf2, p=self.p)*betabinom.pmf(k=self.X[ii],n=self.Y[iii], a=a, b=b)
                
                    indep_noise1 = nbinom.pmf(self.Y, n=minf2, p=self.p).reshape([1,1,1,qMABCD.shape[3]])
                    ...
                    indep_noise1 = nbinom.pmf(self.Y, n=minf2, p=self.p).reshape([1,1,1,1,qMABCD.shape[4]])
                    indep_noise2 = nbinom.pmf(self.X, n=minf1, p=self.p).reshape([1,1,qMABCD.shape[2],1,1])
                    qMABCD = np.multiply(qMABCD, indep_noise1)        
                    qMABCD = np.multiply(qMABCD,indep_noise2)
...
bias_grid =[True]*10 #[False,False,True,True,False,False,False,False, False, True] #[False,False,False,False,False,True]
n_trials = 25
max_iters = 500
verbose = False
savecwd = os.path.join(os.getcwd(),'NB-PID-Results')
...
for f in range(len(func1_lst)):
    ...
    for dim_M in [2,4]:
        if not LOAD_DATA_FLAG:
            #### Specify PMXY
            M = np.random.uniform(low=5, high=10, size=(n_trials,dim_M,))
            p_m = np.random.exponential(scale=1, size=(n_trials,dim_M))
            p_m = p_m
            p_m = p_m/np.sum(p_m, axis=1).reshape(-1,1)
            
            nb_p = np.random.uniform(low=0.3,high=0.7, size=(n_trials))
            pid_calc = []
            num_cores = np.min([n_trials,5])
            summ_stats = []
            true_flag = 0
            for j in range(n_trials//num_cores):
                ...
                for n in range(num_cores):
                    ...
                    #### Quantize X
                    X_quant = np.array([[nbinom.ppf(0.01, n=func1_lst[f](M[n+j*num_cores,i]), p=nb_p[n+j*num_cores]), nbinom.ppf(0.99,n=func1_lst[f](M[n+j*num_cores,i]), p=nb_p[n+j*num_cores])] for i in range(dim_M)])
                    min_X, max_X = np.min(X_quant[:,0]), np.max(X_quant[:,1])+10
                    min_X = 0
                    X_quant = np.arange(int(min_X), int(max_X)+1)

                    #### Quantize Y
                    Y_quant = np.array([[nbinom.ppf(0.01, n=func2_lst[f](M[n+j*num_cores,i]), p=nb_p[n+j*num_cores]), nbinom.ppf(0.99, n=func2_lst[f](M[n+j*num_cores,i]), p=nb_p[n+j*num_cores])] for i in range(dim_M)])
                    min_Y, max_Y = np.min(Y_quant[:,0]), np.max(Y_quant[:,1])+10
                    min_Y = 0
                    Y_quant = np.arange(int(min_Y), int(max_Y)+1) 
                    
                    #### Calc pX|M
                    pXgM = np.array([nbinom.pmf(X_quant, n=func1_lst[f](M[n+j*num_cores,i]), p=nb_p[n+j*num_cores]) for i in range(dim_M)])
                    pXgM = pXgM/np.sum(pXgM,axis=1).reshape(-1,1)
                    
                    #### Calc pY|M
                    pYgM = np.array([nbinom.pmf(Y_quant, n=func2_lst[f](M[n+j*num_cores,i]), p=nb_p[n+j*num_cores]) for i in range(dim_M)])
                    ...
                    pid_calc.append(calc_NB_PID.remote(pMXY=pMXY, f1=func1_lst[f], f2=func2_lst[f], M=M[n+j*num_cores], X=X_quant, Y=Y_quant, p=nb_p[n+j*num_cores], bias_grid=bias_grid[f]))
